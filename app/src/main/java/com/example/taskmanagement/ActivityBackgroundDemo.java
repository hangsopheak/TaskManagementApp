...
import com.example.taskmanagement.background.QuoteGenerateWorker;
import com.example.taskmanagement.background.UploadLogWorker;
...
public class ActivityBackgroundDemo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Schedule Periodic Quote Generate
        binding.btnPeriodicWork.setOnClickListener(v -> {
            PeriodicWorkRequest quoteWork = new PeriodicWorkRequest.Builder(QuoteGenerateWorker.class, 15, TimeUnit.MINUTES)
        ...
        binding.btnConstrainedWork.setOnClickListener(v -> {
            ...
            OneTimeWorkRequest uploadLogs = new OneTimeWorkRequest.Builder(UploadLogWorker.class)
                    .setConstraints(constraints)
                    .build();

            WorkManager.getInstance(this).enqueue(uploadLogs);

            Toast.makeText(this, "📤 Log upload scheduled (Wi-Fi)", Toast.LENGTH_SHORT).show();
        });
    }
